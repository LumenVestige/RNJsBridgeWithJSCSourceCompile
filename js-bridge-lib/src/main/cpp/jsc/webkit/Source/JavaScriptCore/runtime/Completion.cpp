...
{
    TRACE_SECTION("inline bool checkSyntaxInternal");
...
{
    TRACE_SECTION("bool checkSyntax(ExecState* exec, const SourceCode& source, JSValue* returnedException)");
    VM& vm = exec->vm();
...
{
    TRACE_SECTION("bool checkSyntax(VM& vm, const SourceCode& source, ParserError& error)");
    JSLockHolder lock(vm);
    RELEASE_ASSERT(vm.atomStringTable() == Thread::current().atomStringTable());
...
{
    TRACE_SECTION("RefPtr<CachedBytecode> generateProgramBytecode(VM& vm, const SourceCode& source, FileSystem::PlatformFileHandle fd, BytecodeCacheError& error)");
    JSLockHolder lock(vm);
    RELEASE_ASSERT(vm.atomStringTable() == Thread::current().atomStringTable());
...
{
    TRACE_SECTION("RefPtr<CachedBytecode> generateModuleBytecode");
    JSLockHolder lock(vm);
    RELEASE_ASSERT(vm.atomStringTable() == Thread::current().atomStringTable());
...
{
    TRACE_SECTION("JSValue evaluate");
    VM& vm = exec->vm();
    JSLockHolder lock(vm);
...
{
    TRACE_SECTION("JSValue profiledEvaluate");
    VM& vm = exec->vm();
...
{
    TRACE_SECTION("JSValue evaluateWithScopeExtension");
    VM& vm = exec->vm();
...
{
    TRACE_SECTION("static JSInternalPromise* rejectPromise");
    VM& vm = exec->vm();
    auto scope = DECLARE_CATCH_SCOPE(vm);
